...
class PatchOrderPaymentConfirmationApiTest {
  ...
  @Autowired
  PatchOrderPaymentConfirmationApiTest(MockMvc mockMvc, EntityManager entityManager) {
  ...
  void shouldReturnBadRequestWhenOrderIdIsInvalid() throws Exception {
    ...
    var orderInputDto = JsonUtil.toJson(order);
    var orderId = "1aB";
    var request = patch(URL_ORDERS_PAYMENT_CONFIRMATION, orderId)
        .contentType(APPLICATION_JSON)
        .content(orderInputDto);
    mockMvc.perform(request)
  ...
  void shouldReturnBadRequestWhenOrderIsAlreadyPaid() throws Exception {
    ...
    var request = patch(URL_ORDERS_PAYMENT_CONFIRMATION, orderPaymentConfirmation.getId())
        .contentType(APPLICATION_JSON)
        .content(orderInputDto);
    mockMvc.perform(request)
        .andExpect(status().isBadRequest());
